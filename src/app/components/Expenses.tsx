...
export function Expenses() {
  const { expenses, budgets, addExpense, updateExpense, deleteExpense } = useFinance();
  ...
  // ✅ categorías únicas (evita duplicados)
  const categories = useMemo(() => {
    const unique = new Set(budgets.map((b) => b.category));
              ...
              {expenses.map((item) => (
                <tr key={item.id} className="border-t hover:bg-gray-50">
                  ...
                  <td className="p-3 text-right">
                    ...
                    <button onClick={() => deleteExpense(item.id)} className="text-red-600">
